...
class ChatState extends AppState {
  ...
  void databaseInit(String userId, String myId) async {
    ...
    if (_channelName == null) {
      _channelName = getChannelName(userId, myId);
    }
    kDatabase
        .child("chatUsers")
        .child(myId)
    ...
    final channelName = getChannelName(userId, myId);
    if (messageQuery == null || _channelName != channelName) {
      ...
      kDatabase
        ...
        if (snapshot.snapshot.value != null) {
          ...
          map.forEach((key, value) {
            var model = ChatMessage.fromJson(Map<String, dynamic>.from(value as Map));
  ...
  String getChannelName(String user1, String user2) {
    user1 = user1.length >= 5 ? user1.substring(0, 5) : user1;
    user2 = user2.length >= 5 ? user2.substring(0, 5) : user2;
  ...
  void onChatScreenClosed() {
    if (_chatUserList != null && _chatUserList!.isNotEmpty) {
      var user = _chatUserList!.firstWhere((x) => x.key == chatUser.userId, orElse: () => _chatUserList!.first);
  ...
  void sendAndRetrieveMessage(ChatMessage model) async {
    ...
    await http.post(Uri.parse('https://fcm.googleapis.com/fcm/send'),
        ...
        body: body);
    if (response.reasonPhrase?.contains("INVALID_KEY") ?? false) {
      cprint(
